...
export default function AdminDashboard() {
  ...
  return <main id="top" className="admin-main">
    <section className="admin-head"><div><span className="overline">ADMIN / PRODUCT HEALTH</span><h1>管理ダッシュボード</h1><p>FDE Radar が読まれ、判断と行動につながっているかを確認します。</p></div><div className="range-tabs">{[7, 30, 90].map((range) => <button key={range} className={days === range ? 'active' : ''} onClick={() => setDays(range)}>{range}日</button>)}</div></section>
    ...
    {!loading && !data?.error && <>
      <section className="metric-grid">
        <div><span>ページ表示</span><strong>{number(metrics?.page_views)}</strong><small>計測した閲覧</small></div>
        <div><span>記事を開いた回数</span><strong>{number(metrics?.article_opens)}</strong><small>詳しい判断を確認</small></div>
        <div><span>出典クリック</span><strong>{number(metrics?.source_clicks)}</strong><small>一次情報へ移動</small></div>
        <div><span>保存された回数</span><strong>{number(metrics?.bookmark_saves)}</strong><small>判断材料として保存</small></div>
        <div><span>登録ユーザー</span><strong>{number(data?.users?.total_users)}</strong><small>期間内 +{number(data?.users?.new_users)}</small></div>
        <div><span>収集成功率</span><strong>{Number(data?.ingest?.successRate ?? 100).toFixed(1)}%</strong><small>{number(data?.ingest?.runs)} 回の収集</small></div>
      </section>
      <section className="admin-panel chart-panel"><div className="panel-head"><div><span>USAGE TREND</span><h2>利用の推移</h2></div><small>計測開始後の集計</small></div><TrendChart rows={data?.trend} /></section>
      <section className="admin-panel"><div className="panel-head"><div><span>POPULAR SIGNALS</span><h2>よく使われた記事</h2></div></div>
