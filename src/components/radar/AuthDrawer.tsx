  ...
  return <>
    ...
    <aside className={`account-drawer ${open ? 'is-open' : ''}`} aria-hidden={!open} inert={!open ? true : undefined}>
      <div className="drawer-head">
        <div><span>ACCOUNT</span><h2>{user ? 'アカウント' : 'FDE Radar に参加'}</h2></div>
      ...
      {user ? <div className="account-body">
        ...
        {user.isAdmin && <button className="drawer-action admin-drawer-link" onClick={onAdmin}><Icon name="map" />管理画面を開く<Icon name="chevron" /></button>}
