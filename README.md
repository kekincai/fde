...
`src/styles/global.css` は既存の画面・操作構造、`src/styles/editorial.css` は全画面共通の配色、文字サイズ、余白とレスポンシブ表示を担当します。後者を後に読み込みます。暖白の背景、墨色の本文、青のアクセントを共通トークンで管理します。

- ホーム: FDE の定義、現場の循環、収集情報と知識マップへの入口。
- 知識マップ: 5つの段階を縦に並べ、関連する問いをデスクトップで2列、モバイルで1列に表示。
- 収集情報: 一覧では概要を2行、展開後は全文を表示。選択した章名を一覧の見出しに反映。
- モバイル: 下部ナビゲーションとセーフエリアを確保し、操作ボタンと本文の可読性を優先。
- 動きを減らす設定では、アニメーションとスクロール演出を停止。

変更時はホーム、知識マップ、記事の展開、アカウント、管理画面への影響を確認し、少なくとも390pxとデスクトップ幅で表示を確認してください。
